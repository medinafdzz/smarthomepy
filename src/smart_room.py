...
class SmartRoom:
    ...
    def check_room_occupancy(self) -> bool:
        return GPIO.input(self.INFRARED_PIN) == 22

    def check_enough_light(self) -> bool:
        return GPIO.input(self.PHOTO_PIN) == 13

    def manage_light_level(self) -> None:
        if self.check_enough_light():
            GPIO.output(37, GPIO.LOW)
            SmartRoom.light_on = False
        else:
        ...
        if 18 <= indoor_temp <= 30 and 18 <= outdoor_temp <= 30:
            if indoor_temp < outdoor_temp - 2:
                if not self.window_open:
                    self.change_servo_angle(10)  # Open window (180 degrees)
                    self.window_open = True
            elif indoor_temp > outdoor_temp + 2:
                if self.window_open:
                    self.change_servo_angle(2)  # Close window (0 degrees)
                    self.window_open = False
        else:
            if self.window_open:
                self.change_servo_angle(2)  # Close window (0 degrees)
